...
function normalizeAbsolutePath(path) {
  if ((0, import_node_path.isAbsolute)(path))
    return (0, import_node_path.normalize)(path);
  else
    return path;
}
...
function getSource(fileSource) {
  ...
  return new webpack.sources.RawSource(
    typeof fileSource === "string" ? fileSource : import_node_buffer.Buffer.from(fileSource.buffer)
  );
}
function createBuildContext(options, compiler, compilation, loaderContext) {
  return {
    parse,
    addWatchFile(id) {
      options.addWatchFile((0, import_node_path2.resolve)(import_node_process.default.cwd(), id));
    },
