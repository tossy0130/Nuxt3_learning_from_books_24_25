...
// node_modules/.pnpm/tsup@8.3.5_jiti@2.4.1_postcss@8.4.49_tsx@4.19.2_typescript@5.6.3_yaml@2.6.1/node_modules/tsup/assets/cjs_shims.js
...
function guessLoader(code, id) {
  return ExtToLoader[import_node_path.default.extname(id).toLowerCase()] || "js";
}
  ...
  if (!Object.prototype.hasOwnProperty.call(map, "toUrl")) {
    Object.defineProperty(map, "toUrl", {
      ...
      value: function toUrl() {
        return `data:application/json;charset=utf-8;base64,${import_node_buffer.Buffer.from(this.toString()).toString("base64")}`;
      }
    });
  }
  return map;
  ...
  return {
    ...
    emitFile(emittedFile) {
      ...
      if (initialOptions.outdir && emittedFile.source && outFileName) {
        const outPath = import_node_path.default.resolve(initialOptions.outdir, outFileName);
        const outDir = import_node_path.default.dirname(outPath);
        if (!import_node_fs.default.existsSync(outDir))
          import_node_fs.default.mkdirSync(outDir, { recursive: true });
        import_node_fs.default.writeFileSync(outPath, emittedFile.source);
      }
    },
    getWatchFiles() {
      return watchFiles;
    ...
    return (_build) => {
      ...
      if (plugin.load) {
        onLoad({ filter: onLoadFilter }, async (args) => {
          ...
          const { errors, warnings, mixedContext } = createPluginContext(context);
          const resolveDir = import_node_path2.default.dirname(args.path);
          let code, map;
      ...
      if (plugin.esbuild?.setup)
        return plugin.esbuild.setup(build2);
    };
  };
}
    ...
    emitFile(emittedFile) {
      ...
      if (emittedFile.source && outFileName) {
        context.emitFile({
          ...
          name: outFileName,
          content: [...import_node_buffer2.Buffer.from(emittedFile.source)],
          resourceType: (0, import_node_path3.extname)(outFileName)
        });
      }
    },
    getWatchFiles() {
...
function guessIdLoader(id) {
  return ExtToLoader2[import_node_path4.default.extname(id).toLowerCase()] || "js";
}
...
function encodeStr(str) {
  ...
  return import_node_path4.default.posix.normalize(result.join(""));
}
...
function removeQuery(pathe) {
  ...
  if (queryIndex !== -1) {
    return import_node_path4.default.posix.normalize(pathe.slice(0, queryIndex));
  }
  return import_node_path4.default.posix.normalize(pathe);
}
  ...
  if (plugin.resolveId) {
    ...
    farmPlugin.resolve = {
      ...
      async executor(params, context) {
        const resolvedIdPath = import_node_path5.default.resolve(
          process.cwd(),
          params.importer ?? ""
        );
        let isEntry = false;
        ...
        if (!isStartsWithSlash(params.source))
          return null;
        const rootAbsolutePath = import_node_path5.default.resolve(
          params.source
        );
        if ((0, import_node_fs3.existsSync)(rootAbsolutePath)) {
          return {
...
function normalizeAbsolutePath(path5) {
  if ((0, import_node_path6.isAbsolute)(path5))
    return (0, import_node_path6.normalize)(path5);
  else
    return path5;
}
...
function createBuildContext2(compiler, compilation, loaderContext) {
  return {
    ...
    addWatchFile(file) {
      compilation.fileDependencies.add((0, import_node_path7.resolve)(process.cwd(), file));
    },
    getWatchFiles() {
      ...
      if (emittedFile.source && outFileName) {
        ...
        compilation.emitAsset(
          ...
          new sources.RawSource(
            typeof emittedFile.source === "string" ? emittedFile.source : import_node_buffer3.Buffer.from(emittedFile.source)
          )
        );
      }
    }
...
function encodeVirtualModuleId(id, plugin) {
  return (0, import_node_path8.resolve)(plugin.__virtualModulePrefix, encodeURIComponent(id));
}
function decodeVirtualModuleId(encoded, _plugin) {
  return decodeURIComponent((0, import_node_path8.basename)(encoded));
}
function isVirtualModuleId(encoded, plugin) {
  return (0, import_node_path8.dirname)(encoded) === plugin.__virtualModulePrefix;
}
var FakeVirtualModulesPlugin = class {
  ...
  async writeModule(file) {
    ...
    await import_node_fs4.default.promises.writeFile(path5, "");
    return path5;
  }
};

// src/rspack/index.ts
var TRANSFORM_LOADER = (0, import_node_path9.resolve)(
  __dirname,
  false ? "../../dist/rspack/loaders/transform.js" : "rspack/loaders/transform"
);
var LOAD_LOADER = (0, import_node_path9.resolve)(
  __dirname,
...
function getRspackPlugin(factory) {
  return (userOptions) => {
    return {
      apply(compiler) {
        const VIRTUAL_MODULE_PREFIX = (0, import_node_path9.resolve)(compiler.options.context ?? process.cwd(), "node_modules/.virtual");
        const meta = {
          framework: "rspack",
          rspack: {
            compiler
...
function getSource(fileSource) {
  ...
  return new webpack.sources.RawSource(
    typeof fileSource === "string" ? fileSource : import_node_buffer4.Buffer.from(fileSource.buffer)
  );
}
function createBuildContext3(options, compiler, compilation, loaderContext) {
  return {
    parse,
    addWatchFile(id) {
      options.addWatchFile((0, import_node_path10.resolve)(import_node_process.default.cwd(), id));
    },
    emitFile(emittedFile) {
      const outFileName = emittedFile.fileName || emittedFile.name;
      if (emittedFile.source && outFileName) {
...
// src/webpack/index.ts
var TRANSFORM_LOADER2 = (0, import_node_path11.resolve)(
  __dirname,
  false ? "../../dist/webpack/loaders/transform" : "webpack/loaders/transform"
);
var LOAD_LOADER2 = (0, import_node_path11.resolve)(
  __dirname,
...
function getWebpackPlugin(factory) {
  return (userOptions) => {
    return {
      apply(compiler) {
        const VIRTUAL_MODULE_PREFIX = (0, import_node_path11.resolve)(compiler.options.context ?? import_node_process2.default.cwd(), "_virtual_");
        const meta = {
          framework: "webpack",
          webpack: {
            compiler
